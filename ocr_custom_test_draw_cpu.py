...
# Fix for PyInstaller + NumPy
# Special handling for compiled EXE
if getattr(sys, 'frozen', False):
    # Add numpy and torch to path
    sys.path.insert(0, os.path.join(sys._MEIPASS, 'numpy'))
    sys.path.insert(0, os.path.join(sys._MEIPASS, 'torch'))
    
    # Force numpy initialization
    os.environ['NUMPY_IMPORT'] = 'FORCE'
    import numpy as np
    np.seterr(all='ignore')
    
    # Force torch CPU mode
...
# Legacy CPU enforcement (for PyTorch 1.x)
torch.set_default_tensor_type(torch.FloatTensor)  # Works in all versions
device = torch.device('cpu')  # Explicit device setting
...
# Remove old tensor type setting (deprecated)
# torch.set_default_tensor_type(torch.FloatTensor)  # Remove this line
